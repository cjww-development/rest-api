...
trait BackController extends Controller with ConfigurationStrings {
  ...
  protected def decryptRequest[T](f: (T) => Future[Result])(implicit request: Request[String], manifest : Manifest[T], reads : Reads[T], format : Format[T]) = {
    Try(JsonSecurity.decryptInto[T](request.body)) match {
      ...
      case Failure(e) =>
        Logger.error(s"[BackController] - [decryptRequest] Request body decryption has FAILED - reason : ${e.getStackTrace}")
  ...
  private def checkAuth(appID : Option[String]) : AuthorisationResponse = {
    appID match {
      case Some(id) => id match {
        case AUTH_ID => Logger.info("[BackController] - [checkAuth] : API call established, Welcome AUTH SERVICE")
          Authorised
        case DIAG_ID => Logger.info("[BackController] - [checkAuth] : API call established, Welcome DIAGNOSTICS FRONTEND")
          Authorised
        case DEV_ID => Logger.info("[BackController] - [checkAuth] : API call established, Welcome DEVERSITY FRONTEND")
          Authorised
        case _ => NotAuthorised
      }
      case None => Logger.warn("[BackController] - [checkAuth] : API CALL FROM UNKNOWN SOURCE - ACTION DENIED")
        NotAuthorised
    }
  }
}
